...
> **重要**：为微信公众号生成插图时，必须遵循 [微信公众号插图设计规范](wechat-design-spec.md)。
...
2. **模板选择**
   - 根据文章类型选择对应的配色方案和模板
   - 参考模板文件：`templates/wechat-cover-{type}.html`
   - 确保尺寸符合微信公众号规范
...
5. **导出与命名**
   - 按规范命名文件：`{文章ID}_{插图类型}_{尺寸}_{序号}.{格式}`
   - 控制文件大小在限制范围内
   - 选择合适的导出格式（PNG/JPG）
...
**文中插图**
- **概念图（900×600）**：清晰表达核心概念，适合技术教程、观点类
- **流程图（900×自适应）**：步骤清晰、逻辑连贯，适合技术教程
- **对比图（900×400）**：对比鲜明、差异突出，适合观点类、产品发布
- **数据图（900×500）**：数据准确、易于理解，适合资讯类、产品发布
- **时间线（900×300）**：时间节点清晰，适合故事类、资讯类

**品牌元素**
- **头图（900×300）**：文章顶部品牌展示，包含Logo和核心数据
- **尾图（900×200）**：文章底部二维码/引导，包含联系方式
...
详细的微信公众号插图设计规范请参考：[wechat-design-spec.md](wechat-design-spec.md)

该规范包含：
- 微信生态视觉风格指南（色彩系统、字体规范、构图原则）
- 插图尺寸标准（封面图、文中插图、头图等）
- 文章类型分类与插图模板
- 文件命名与导出规范
- 内容匹配度评估标准
...
| 文件名 | 用途 | 尺寸 |
|--------|------|------|
| wechat-cover-large.html | 封面大图 | 900×383 |
| wechat-cover-medium.html | 封面次图 | 900×500 |
| wechat-cover-small.html | 封面小图 | 200×200 |
| wechat-cover-news.html | 资讯类封面 | 900×383 |
| wechat-cover-opinion.html | 观点类封面 | 900×383 |
| wechat-cover-story.html | 故事类封面 | 900×383 |
| wechat-cover-tutorial.html | 技术教程封面 | 900×383 |
| wechat-cover-release.html | 产品发布封面 | 900×383 |
| wechat-cover-event.html | 活动推广封面 | 900×383 |
| wechat-inline.html | 文中插图 | 900×600 |
| wechat-header.html | 头图 | 900×300 |
| wechat-footer.html | 尾图 | 900×200 |

**通用模板**
| 文件名 | 用途 | 尺寸 |
|--------|------|------|
| horizontal-template.html | 横向图片 | 1280×720 |
| vertical-template.html | 纵向图片 | 720×1280 |