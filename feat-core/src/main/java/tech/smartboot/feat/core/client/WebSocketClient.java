...
//                throw throwable;
    ...
    private void initRest() throws IOException {
        ...
        request.addHeader(HeaderName.UPGRADE.getName(), HeaderValue.Upgrade.WEBSOCKET);
        request.setHeader(HeaderName.CONNECTION.getName(), HeaderValue.Connection.UPGRADE);
        request.setHeader(HeaderName.Sec_WebSocket_Key.getName(), generateSecWebSocketKey());
        request.setHeader(HeaderName.Sec_WebSocket_Version.getName(), "13");
