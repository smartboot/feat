 ...
 public class DataSourceSerializer extends ExtensionSerializer {
     ...
     public void serializeLoadBean() {
         Object driver = JSONPath.eval(config, "$.feat.datasource.driver");
         Object url = JSONPath.eval(config, "$.feat.datasource.url");
         Object username = JSONPath.eval(config, "$.feat.datasource.username");
         Object password = JSONPath.eval(config, "$.feat.datasource.password");
         printWriter.append(headBlank(0)).println("DataSource dataSource = new PooledDataSource(" + SerializerUtils.resolveStringProperty(driver) + ", " + SerializerUtils.resolveStringProperty(url) + ", " + SerializerUtils.resolveStringProperty(username) + ", " + SerializerUtils.resolveStringProperty(password) + ");");
