...
public class RestfulServer2Test extends BastTest {
    ...
    public void testPostJson() throws ExecutionException, InterruptedException {
        ...
        Assert.assertEquals(httpClient.post("/demo2/param3").header(h->h.setContentType(HeaderValue.ContentType.APPLICATION_JSON)).body().write(jsonObject.toJSONString().getBytes()).done().done().get().body(), "hello paramPost1 paramPost2");
    }
    ...
    public void testPostJson1() throws ExecutionException, InterruptedException {
        ...
        jsonObject.put("param1", "paramPost1");
        jsonObject.put("param2", "paramPost2");
        Assert.assertEquals(httpClient.post("/demo2/param4").header(h->h.setContentType(HeaderValue.ContentType.APPLICATION_JSON)).body().write(jsonObject.toJSONString().getBytes()).done().done().get().body(), "hello param is null");
    }
    ...
    public void testPostJson2() throws ExecutionException, InterruptedException {
        ...
        jsonObject.put("param1", "paramPost1");
        jsonObject.put("param2", "paramPost2");
        Assert.assertEquals(httpClient.post("/demo2/param5").header(h->h.setContentType(HeaderValue.ContentType.APPLICATION_JSON)).body().write(jsonObject.toJSONString().getBytes()).done().done().get().body(), "hello param is null");
    }
