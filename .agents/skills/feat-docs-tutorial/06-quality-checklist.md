...
**示例：**
```
故事：开发者小明需要实现用户认证，从最简单的 Session 方案开始，
      发现分布式环境下的问题，最终使用 JWT 解决。

情节点：
1. 场景引入：小明的单体应用需要登录功能（共鸣）
2. 初次尝试：使用 Session 实现（成就感）
3. 遇到问题：部署多实例后登录失效（困惑）
4. 探索方案：了解 Session 共享 vs JWT（启发）
5. Feat 实现：使用 Feat 的 JWT 支持（成就感）
6. 效果验证：分布式环境测试通过（验证）
```
