...
**步骤 2：获取并缓存**
- 如果本地缓存不存在，使用网络搜索工具获取 `https://smartboot.tech/feat/` 官方文档的最新内容
- **缓存的内容必须严格来自 `https://smartboot.tech/feat` 站点，确保信息的准确性和权威性**
- **必须先将获取到的内容写入本地缓存文件**，再基于缓存内容回答问题
- 将内容整理为 Markdown 格式，使用 Write 工具写入 `.knowledge/{category}/{topic}.md`

**步骤 3：基于缓存处理**
- 从本地缓存文件读取内容，用于回答用户问题
- 确保缓存成功且内容来源正确后再进行后续处理
