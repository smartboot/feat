...
如果只是服务端持续推送，而客户端不需要主动发消息，先看 [SSE 客户端](/feat/client/sse_client/)。
...
那通常不是 WebSocket 的最佳入口，先看 [SSE 客户端](/feat/client/sse_client/)。

## 下一步

- [HttpClient 实战教程](/feat/client/http_client/)：如果你的通信其实还是普通 HTTP
- [SSE 客户端](/feat/client/sse_client/)：如果你的场景只是服务端推送
