...
## 什么时候用 Feat HttpClient

Feat HttpClient 适合以下场景：
...
- **团队已深度使用 Spring** 且需要无缝集成 → Spring 生态的 HttpClient 集成更好

<Aside type="tip">
Feat HttpClient 的设计哲学是**简单高效**。相比 WebFlux 的 `Mono`/`Flux`，Feat 使用标准的 `CompletableFuture`，学习成本更低，更容易上手。如果你需要异步 HTTP 客户端但不想引入复杂的响应式编程模型，Feat 是一个很好的选择。
</Aside>
