...
- 升级 fastjson 版本至 `2.0.64`

#### Feat Cloud / AOT

- 修复 `feat.yaml` 缺失 `feat.redis.username` 配置解析的问题。
- Controller 增加返回类型适配： `List<Byte>`、`List<Short>`、`List<Integer>`、`List<Long>` 以及直接继承 Map 的POJO 类。
- 修复通过反射注入代理 Bean 属性字段报 `NoSuchFieldException` 的问题。
- 优化 AOT 编译失败的异常提示
- 环境变量与系统属性支持
    - feat.yml 支持 `${}` 语法：`${property}` 或 `${property:defaultValue}`
    - 解析优先级：系统属性 > 环境变量 > 默认值
    - 支持数据源、Redis、服务器等所有配置项
- 序列化工具优化
    - 新增 SerializerUtils 工具类统一属性解析
    - 重构序列化器，使用 IntegerValuesSerializer 和 StringValuesSerializer 统一处理数组和列表
    - 增强 null 值处理


### 升级步骤
<Steps>
1. 更新版本号：`<feat.version>2.4.0</feat.version>`
2. 在 `feat.yml` 中使用 `${}` 语法（可选）
3. 通过系统属性或环境变量传入配置值：

    ```bash java -Ddatasource.url=jdbc:mysql://localhost/feat -jar app.jar ```
</Steps>
### 兼容性说明
- ✅ 完全向后兼容 v2.3.0
- ✅ 旧配置方式继续有效
...
- **HTTP**：优化 `PostInputStream` 读取逻辑，修复完整 body 读取时可能触发的 `StackOverflowError` 异常
- **HTTP**：调整异步请求体读取缓冲区判断逻辑，提升小 body 场景的读取性能
- **依赖**：`smart-socket` 升级至 `2.1.3`

#### Feat Cloud / AOT

- **拦截器**：新增方法拦截器能力，支持 `@Interceptor`、`@AroundInvoke` 和 `@Interceptors`
- **拦截器**：优化拦截器链异常处理，并完善 AOT 序列化支持
- **示例**：新增 Nacos 配置中心接入示例，补充多环境配置与动态刷新用法
- **Bean**：`@Bean` 默认加载顺序调整为 `Byte.MAX_VALUE`，方便基础设施 Bean 提前初始化
...
1. **更新版本号**
   ...
   ```xml
   <feat.version>2.3.0</feat.version>
   ```

2. **检查 Bean 初始化顺序**

   `@Bean` 的默认 `order` 从 `0` 调整为 `Byte.MAX_VALUE`。如果基础设施 Bean 需要优先初始化，请显式设置较小的顺序：
...
3. **重新编译 AOT 生成代码**
   ...
   如果新增或调整了 `@Interceptor`、`@AroundInvoke`、`@Interceptors`，需要重新执行 `mvn compile` 或重新打包，让 AOT 代码重新生成。
</Steps>
...
| 变更项 | 影响 | 处理方式 |
|--------|------|----------|
| `@Bean` 默认 `order` | 默认 Bean 初始化顺序由 `0` 变为 `Byte.MAX_VALUE` | 依赖初始化顺序的 Bean 显式声明 `order` |
| 方法拦截器 | 新增编译期 AOT 能力，不影响未使用拦截器的应用 | 使用拦截器后重新编译生成代码 |
| `smart-socket` 版本 | 默认依赖升级至 `2.1.3` | 如项目显式锁定 smart-socket 版本，建议同步升级 |
