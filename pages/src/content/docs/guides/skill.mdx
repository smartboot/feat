...
在使用 Cursor、Windsurf、Claude Code 等 AI 编程工具时，AI 对 Feat 的了解可能不够深入，导致给出的代码建议不够准确。

**Skill 解决了这个问题**：通过安装 Feat 官方 Skill，AI 工具可以自动学习 Feat 官方文档，从而：
...
将 Skill 文件放入项目的 `.agents/skills` 目录即可：
...
<Aside type="note">
不同 AI 工具的 Skill 加载方式可能略有差异，但通常只需将 SKILL.md 文件放入项目的 `.agent/skills` 目录，AI 工具会自动识别并加载。
</Aside>
...
Feat Skill 涵盖官方文档的所有主要内容：

| 模块 | 内容 | 适用场景 |
|------|------|----------|
| 入门指南 | 环境搭建、第一个项目、基础概念 | 初学者快速上手 |
| Feat Core | HTTP 服务器、路由、中间件、过滤器 | 构建 Web 服务 |
| Feat Cloud | 控制器、依赖注入、数据库、配置 | 构建企业级应用 |
| Feat AI | AI 集成、智能代理、对话系统 | 集成 AI 功能 |
| 客户端 | HTTP 客户端、WebSocket 客户端 | 调用外部服务 |
| 最佳实践 | 性能优化、安全建议、部署指南 | 生产环境优化 |

## 提问技巧

<Aside type="tip">
提供具体的场景和上下文，AI 才能给出最准确的回答。
</Aside>

**推荐的提问方式：**

```
@feat-tutorial 我正在开发一个 REST API，需要：
1. 处理 JSON 请求和响应
2. 实现路由分发
3. 添加全局异常处理
...
你也可以直接复制以下内容，保存为 `SKILL.md` 文件后放入 `.agents/skills/feat-tutorial/` 目录：
```markdown
---
name: "feat-tutorial"
