...
Agent 是 Feat AI 的高级功能，使 AI 能够自主规划、使用工具并执行多步骤任务。

## 概述

传统的 Chat Model 只能进行对话，而 Agent 能够：

- **自主规划** - 分析任务并制定执行计划
- **使用工具** - 调用搜索、文件操作、计算等外部能力
- **多轮迭代** - 通过观察结果持续优化行动
- **完成复杂任务** - 自动处理需要多个步骤的目标

Feat AI 的 Agent 基于 **ReAct（Reasoning + Acting）** 范式，让 AI 像人一样**思考 → 行动 → 观察 → 再思考**。
...
Agent:
  ...
  Final Answer: Java 21 的主要新特性包括：1. 虚拟线程正式发布...
```
...
| 组件 | 职责 | 关键类 |
|------|------|--------|
| **Agent** | 任务调度、推理循环 | `FeatAgent`, `ReActAgent` |
...
FeatAgent agent = new ReActAgent(opts -> {
    opts.chatOptions.model(ChatModelVendor.GiteeAI.DeepSeek_V32);
});
...
<Aside type="note">
`chatOptions` 是 `AgentOptions` 的字段（类型为 `ChatOptions`），使用 `.` 访问而非 `()`。
</Aside>
...
```java
FeatAgent agent = new ReActAgent(opts -> {
    // 通过 toolExecutors Map 注册工具
    opts.toolExecutors.put("calculator", new CalculatorTool());
    opts.toolExecutors.put("another", new AnotherCustomTool());
});
```

<Aside type="note">
`toolExecutors` 是 `AgentOptions` 的字段，直接通过 `put()` 方法注册工具。
</Aside>

### 异步与并发

Agent 的所有操作都是异步的，返回 `CompletableFuture`：
...
new ReActAgent(opts -> {
    ...
    // 工具通过 toolExecutors Map 注册
    opts.toolExecutors.put("toolName", toolInstance);
    // 模型通过 chatOptions 字段配置
    opts.chatOptions.model(ChatModelVendor.GiteeAI.DeepSeek_V32);
});
```

<Aside type="note">
- `toolExecutors` 是 `Map<String, AgentTool>` 类型的字段
- `chatOptions` 是 `ChatOptions` 类型的字段，用于配置 AI 模型
</Aside>
...
A: 通过 `opts.chatOptions.temperature(0.3f)` 降低随机性，提供更详细的任务描述和示例
