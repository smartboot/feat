...
> 你说："阅读 Spring 官方文档，对比 Spring Boot 2 和 3 的差异，生成一份分析报告"
> 
> Agent 会：
> 1. 搜索 Spring 官方文档
> 2. 读取文档内容
...
public class DatabaseTool implements AgentTool {
    ...
    public String getDescription() {
        return "执行 SQL 查询，参数：sql (string)";
    }
...
```java
ReActAgent agent = new ReActAgent(opts -> {
    opts.addTool(new DatabaseTool());
});
```

现在 Agent 就能使用 `database_query` 工具执行 SQL 查询了。
...
```java
String task = "请审查 ./src/main/java 目录下的代码：\n"
    + "1. 列出所有 Java 文件\n"
    + "2. 读取每个文件的内容\n"
    + "3. 检查是否存在潜在问题（空指针、资源未关闭等）\n"
    + "4. 生成审查报告";

String report = agent.execute(task).get();
```
