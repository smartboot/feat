---
title: Router 路由组件
description: 本教程将带你掌握 Feat Router 的使用方法，包括路由配置、Session 管理和拦截器机制
sidebar:
  order: 3
---
...
- [Feat Server 快速入门](/feat/server/getstart/) - 了解基本的 HTTP 服务器创建
- JDK 8 或更高版本
- 熟悉 Java Lambda 表达式
...
<Aside type="tip" title="Session 类型说明">
  Feat 的 Session 只支持存储 **String 类型的值**。`session.put(key, value)` 方法的 value 参数必须是 String 类型，`session.get(key)` 返回的也是 String 类型。如果需要存储复杂对象，建议先序列化为 JSON 字符串。
</Aside>
```
