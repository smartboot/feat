...
这篇文档解决一个很具体的问题：同一套 Feat Cloud 代码，如何在开发、测试、生产环境使用不同配置。
...
合并后，`server` 这个顶层配置会以 `feat-dev.yml` 中的 `server` 为准：
...
这意味着 `debug` 和 `host` 不会从默认配置里自动保留下来。  
如果你希望它们在 `dev` 环境继续存在，就需要在 `feat-dev.yml` 里一起写出来：
...
这个规则虽然没有深度合并那么“聪明”，但它更明确：环境文件里出现的顶层模块，就是该环境对这个模块的完整声明。
