...
如果你的项目会区分开发、测试、生产环境，可以把配置拆成两层：

- `feat.yml` 作为默认配置
- `feat-dev.yml`、`feat-prod.yml` 作为环境覆盖配置

Feat Cloud 在编译期会先读取默认配置，再把对应环境的配置覆盖上去，最后为每个环境生成一套完整代码。  
默认环境对应的生成类不会带后缀，其他环境会在类名上追加环境名，比如 `FeatApplicationDev`、`UserControllerCloudServiceProd`。
...
运行时通过 `feat.profiles.active` 选择要启用的环境：
...
<Aside type="tip">
如果项目里只有一份默认配置，就不需要额外设置 `feat.profiles.active`。这时生成的代码只会保留默认环境那一套，不会多出 profile 判断。
</Aside>
