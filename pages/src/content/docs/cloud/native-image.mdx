...
GraalVM Native Image 为部署和运行 Java 应用程序提供了一种新方式。

与传统的 Java 虚拟机相比，原生镜像可以更小的内存占用运行，并具有更快的启动速度。它们非常适合使用容器镜像部署的应用程序，特别是与"函数即服务"(FaaS)平台结合使用时。
...
Feat 框架通过其独特的 AOT 编译机制，天然适合 GraalVM Native Image 构建。相比其他框架需要大量配置和调整才能构建原生镜像，Feat 几乎无需额外工作即可构建出高性能的原生镜像应用。
