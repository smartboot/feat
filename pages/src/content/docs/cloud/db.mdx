...
<Aside type="tip">
配置 `initial-sql` 后，Feat Cloud 会在编译期将 SQL 初始化逻辑嵌入生成的代码中，应用启动时自动执行，无需手动编写初始化代码。
</Aside>
...
<Aside type="caution">
当前仅支持 `POOLED` 数据源类型（`PooledDataSource`）和 `JDBC` 事务管理器（`JdbcTransactionFactory`）。
</Aside>
...
Feat Cloud 在编译期解析 `feat.yaml` 和 MyBatis XML 配置，自动生成 `SqlSessionFactory` Bean 的初始化代码。生成的代码包含以下要素：

- **数据源配置**：从 XML 解析的数据源参数（驱动、URL、用户名、密码）
- **事务管理器**：`JdbcTransactionFactory`
- **配置选项**：延迟加载、自动映射行为、默认执行器类型等
- **拦截器**：XML 中配置的拦截器列表
- **映射器注册**：`mappers` 包扫描结果
- **数据库初始化**：`initial-sql` 指定的脚本执行

生成的 `SqlSessionFactory` Bean 名称为 `sessionFactory`，可直接注入使用。

### 创建应用入口

创建主类启动 Feat Cloud 应用：
...
<Aside type="tip">
当配置了 `feat.mybatis.initial-sql` 后，无需手动编写数据库初始化代码，SQL 脚本会在应用启动时自动执行。
</Aside>
