...
> 示例中插件和基础镜像版本号使用占位符，实际使用时请替换为当前稳定版本。
...
FROM eclipse-temurin:${jre.version}-jre-alpine
WORKDIR /feat
...
FROM container-registry.oracle.com/graalvm/native-image:${graalvm.version}-ol8 AS builder

COPY target/helloworld_docker*.jar helloworld.jar

RUN native-image --no-fallback -jar helloworld.jar

FROM ubuntu:${base.image.version}
EXPOSE 8080
