...
Feat框架通过AOT（Ahead-of-Time）编译技术实现零反射的运行时环境，在编译期完成所有代码生成和优化工作。
...
**AOT 虚拟机**(AOT VM) 模式专为开发阶段设计，通过`AotVMCloudService`在运行时模拟AOT编译行为，提供与AOT模式一致的API体验。
...
首先，在`maven-compiler-plugin`中添加以下配置：
...
最后， 在 IDEA 中勾选相应的 profile 以激活该 AOT VM 模式：
