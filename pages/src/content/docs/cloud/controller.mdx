...
- 如果参数不存在且没有默认值，基本类型会报错，包装类型为 `null`
</Aside>
...
- `:paramName` - 冒号形式，如 `/:userId`、 `/users/:userId/orders/:orderId`
- `{paramName}` - 花括号形式，如 `/{userId}`、`/users/{userId}/orders/{orderId}`

两种形式功能等价，可根据团队习惯选择。冒号形式更简洁，花括号形式可读性更强。
</Aside>
...
- [Feat Cloud 快速开始](/feat/cloud/)
- [CloudOptions 配置](/feat/cloud/options/)
- [Router 路由详解](/feat/server/router/)
- [异步处理](/feat/server/async/)
