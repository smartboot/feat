...
这一章从最基础的 Controller 讲起。先把路径、方法和返回值的关系弄清楚，再看 Feat Cloud 为什么能在启动时快速注册路由。
...
当你能稳定新增这样的接口，就可以继续进入参数绑定：让同一个 Controller 不只是返回固定内容，而是能从 URL、路径和请求体里接收输入。
